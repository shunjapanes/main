# コードレビューレポート 2026-07-16 — shunjapanes/main

## サマリー

- 実行日時: 2026-07-16 UTC（自動レビュー・3エージェント並列）
- プロジェクト: TSV/CSVエディタ (React 19 + TypeScript + Vite + Tailwind)
- レビューファイル数: 8件 (App.tsx, bridge.ts, RibbonButton.tsx, RibbonToolbar.tsx, SearchBar.tsx, FileTabBar.tsx, sw.js, package-lock.json)
- 発見件数: 🔴 Critical 1 / 🟠 High 8 / 🟡 Medium 15 / 🟢 Low 17 = 計41件
- うち新規 [NEW]: **5件** / 継続 [継続]: 36件
- 解消 [FIXED]: 0件
- 適用済み自動修正: **0件**（console.log / console.error / debugger 文なし）
- ⚠️ **2026-07-12以降コードに変更なし（4日間）。S-1（Critical）は約1ヶ月未対応継続中。今回はパッケージ脆弱性 CVE 2件（S-17 High・S-18 Low）を新たに検出。**
...
- `btoa` はBase64エンコードであり暗号化ではない。DevToolsから即座に読み取り可能。XSS → PAT漏洩 → リポジトリ書き込みの攻撃チェーンが成立。**約1ヶ月未対応。最優先で対処が必要。**
...
**[NEW] S-17: vite 6.4.2 に既知 CVE 2件**
...
- 本番ビルド成果物への直接影響はないが、Windows 開発機では dev server 経由でホスト上の機密ファイルを読み取られる恐れがある。
- **修正案:** `npm audit fix` で vite 6.4.3+ へ更新（1コマンドで解決）。
...
- `e.key === 'z' && e.shiftKey` はShiftキー押下時 `e.key` が `'Z'`（大文字）になるため条件が永遠にfalse。**1行の修正で解消可能。**
...
**[NEW] S-18: @babel/core 7.29.0 に任意ファイル読み取り CVE**
- 場所: `client/package-lock.json` (node_modules/@babel/core@7.29.0)
- GHSA-4x5r-pxfx-6jf8: sourceMappingURL コメント経由でビルド時に任意ファイルを読み取られる可能性。@vitejs/plugin-react の推移的依存として組み込まれている。
- **修正案:** `npm audit fix` で @babel/core 7.29.1+ へ更新。

**[NEW] Q-N-8: FileReader onload で緩やかな等値演算子 `!= null` を使用**
- 場所: `client/src/App.tsx:111`
- `FileReader.result` の型は `string | ArrayBuffer | null` であり `undefined` を含まないため `!== null` で十分。ESLint eqeqeq ルール警告の対象。
- **修正案:** `if (content !== null)` に変更。

**[NEW] Q-N-9: `type Tab` の命名衝突 — RibbonToolbar と App/FileTabBar で同名の異なる型**
- 場所: `client/src/components/RibbonToolbar.tsx:17`
- `type Tab = typeof TABS[number]`（文字列ユニオン）が `App.tsx` / `FileTabBar.tsx` の `interface Tab { name: string; dirty: boolean }` と同名で定義されている。
- **修正案:** リボンタブ型を `type RibbonTabId = typeof TABS[number]` に改名。

**[NEW] B-11: FileTabBar key={i} — タブ削除時に React の差分アルゴリズムが誤認**
- 場所: `client/src/components/FileTabBar.tsx:19`
- タブA/B/Cのうちインデックス0のタブを閉じると、ReactはB(key=0)を旧Aと同一視し非activeで描画するリスク。メッセージ到達タイミング次第で一瞬誤ったタブがアクティブ表示される。
- **修正案:** エディタ側からユニークIDを付与し `key={tab.id}` とする（Q-6の修正と同時対応）。
...
1. **[即対応・約1ヶ月未対応] S-1 (Critical):** `editor.html` の btoa() localStorage PAT保存を廃止。sessionStorage + セッション変数のみに変更。
2. **[今すぐ・1コマンド] S-17 (High NEW):** `cd client && npm audit fix` で vite 6.4.3+ へ更新。Windows開発機でdev server経由のファイル漏洩リスクあり。
...
5. **[今週・1行修正] B-8 (Medium):** `App.tsx` の FileReader に `reader.onerror = () => setStatus('ファイル読み込みエラー')` を追加。
6. **[今週・1行修正] S-8 (Medium):** `App.tsx` のメッセージハンドラ先頭に `if (e.origin !== window.location.origin) return;` を追加。
7. **[今週] S-12 (Medium):** `editor.html` メッセージハンドラに `e.origin` チェックを追加。
8. **[今週] B-10 (Low):** SearchBar の Escape/X ハンドラで `clearTimeout(debounceRef.current)` を `send('clearSearch')` 前に追加。
9. **[来週] S-18 (Low NEW):** `npm audit fix` で @babel/core 7.29.1+ へ更新（S-17の修正時に同時対応可能）。
10. **[来週] S-13 (Medium):** sw.js を stale-while-revalidate 戦略に変更。
11. **[来週] S-15 (Low):** `App.tsx` の `handleFileSelected` にファイルサイズ上限チェック (50MB) を追加。
12. **[来週] S-14 (Low):** sw.js の `.catch()` で `new Response('Offline', { status: 503 })` フォールバックを返す。

> 詳細な過去経緯は `_review/2026-07-15_code_review.md` を参照
