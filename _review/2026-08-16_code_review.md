# コードレビューレポート 2026-08-16 — shunjapanes/main

## サマリー

- 実行日時: 2026-08-16 00:00 UTC（自動レビュー・3エージェント並列）
...
- うち新規 [NEW]: **6件** / 継続 [継続]: **34件** / 解消 [FIXED]: **0件**
...
| ℹ️ 継続中 | 前回 (2026-08-14) 指摘の全33件が未修正のまま継続中 |
| 🆕 最重要新規 | **NEW-PAT: GitHub PAT を sessionStorage に平文保存** — S-XSS と連鎖し PAT 窃取→リポジトリへの任意アクセスが可能 |
| 🆕 新規発見 | NEW-PAT (High)・NEW-PM2 (Medium)・Q-CAST (Medium)・N-1 (Medium)・NEW-URL (Low)・Q-MOUNT-SEARCH (Low) 計6件 |
...
**[NEW] NEW-PAT: GitHub PAT を sessionStorage に平文保存**
- 場所: `client/public/editor.html:9061`
- `sessionStorage.setItem("tsv-editor-debug-memo-pat", pat)` で GitHub PAT（repo スコープ = 全リポジトリ読み書き権限）を平文保存。editor.html は S-XSS・S-CSS 等の未修正 XSS ベクターを持ち、同一オリジン上の XSS が成立した場合 `sessionStorage.getItem("tsv-editor-debug-memo-pat")` で PAT が即座に窃取される。PAT 奪取後はリポジトリへの任意コミット・機密コードの閲覧が可能。S-5（デバッグUI本番露出）と組み合わさることで攻撃面が増大。
...
- 場所: `client/src/App.tsx:146`
- `window.open()` が editor.html から任意 URL で呼び出し可能。タブナビングやバックグラウンド悪意タブ開封が可能。
- **修正案:** `popups` を allow リストから削除。

**[NEW] NEW-PM2: editor.html から親フレームへの postMessage も `'*'` を使用**
- 場所: `client/public/editor.html:9505, 9539, 9589, 9599, 9609, 9615, 9626, 9630, 9653`
- 前回 S-PM は bridge.ts（親→iframe 方向）の `'*'` を指摘したが、editor.html の全 `window.parent.postMessage(...)` 呼び出しも `'*'` を使用。悪意ある第三者が親フレームを差し替えた場合、ファイル名・ステータス・検索結果・位置情報・トグル状態が漏洩する。
...
**[NEW] NEW-URL: デバッグメモに location.href をそのまま GitHub に公開**
- 場所: `client/public/editor.html:9258`
- `location.href` をメモ本文に含めて GitHub に永続書き込み。クエリパラメータにトークンが含まれる構成では機密漏洩になる。
...
**[NEW] Q-CAST: FileReader.result の unsafe な型キャスト**
- 場所: `client/src/App.tsx:90`
- `ev.target?.result as string` は TypeScript のみのアサーション。null 時も `as string` でキャストされ、後続の `!= null` チェックに依存している。リファクタリング時に型保証が失われる。
...
**[NEW] Q-MOUNT-SEARCH: SearchBar マウント時に send('search','') が発火**
- 場所: `client/src/components/SearchBar.tsx:29-33`
- query を依存配列とする useEffect がマウント時にも実行され、初期値 `''` で `send('search', '')` が 180ms 後に iframe へ送信される。エディタ側の状態を意図せずクリアする可能性がある。
...
**[NEW] N-1: FileReader にファイルサイズ上限チェックなし**
- 場所: `client/src/App.tsx:107-113`
- `reader.readAsText(file)` がサイズ無制限で実行される。数百MB のファイルは JS 文字列に全量読み込まれた後、postMessage でディープコピーされるため、ピーク時のメモリ使用量が2倍になりメインスレッドが長時間フリーズする。キャンセル経路もない。
...
- `.catch(() => cached)` は `cached` が undefined の場合 `undefined` を返し、`respondWith(undefined)` はエラーとなる。
...
- **修正案:** `msg.filterActive ?? toggles.filterActive` で部分更新を安全に行う。
...
1. **[NEW/HIGH] NEW-PAT + S-5**: デバッグ UI を `import.meta.env.DEV` ガードで本番から除外し、sessionStorage の PAT 保存を廃止する（2箇所・即座に実施可能）
2. **[CRITICAL] S-PM + Q-ORIGIN + Q-MSGSRC + NEW-PM2**: postMessage の targetOrigin を `window.location.origin` に統一し、受信側に `e.origin` チェックを追加（bridge.ts・App.tsx・editor.html の各送受信箇所）
3. **[CRITICAL] S-XSS**: `renderHtmlValue` の href に `https?:` ホワイトリスト検証を追加
4. **[HIGH] S-CLIP + S-SBX**: iframe allow から `clipboard-read` を削除し、`sandbox` 属性を最小権限で追加
5. **[HIGH] B-13**: `if (tag === 'IFRAME') return` に INPUT/TEXTAREA/SELECT ガードを追加（1行変更）
6. **[NEW/MEDIUM] N-1**: FileReader 呼び出し前に 50MB ファイルサイズ制限を追加
7. **[HIGH] S-CSP**: `index.html` / `editor.html` に Content-Security-Policy meta タグを追加
8. **[NEW/MEDIUM] NEW-PM2**: editor.html の `window.parent.postMessage` を `location.origin` に統一
9. **[MEDIUM] B-SW + S-SW**: Service Worker の catch フォールバックを `Response('Offline', {status:504})` に修正
10. **[NEW/LOW] NEW-URL**: デバッグメモの `location.href` を `origin+pathname` のみに変更（セキュリティ上の情報漏洩対策）
