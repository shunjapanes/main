# コードレビューレポート 2026-07-26 — shunjapanes/main

## サマリー

- 実行日時: 2026-07-26 16:08 UTC 自動レビュー・3エージェント並列
...
- うち新規 [NEW]: **2件** / 継続 [継続]: **59件** / 解消 [FIXED]: **0件**
...
| 🆕 Q-N-18 [NEW] | `stateSync` ハンドラで省略フィールドが `!!(undefined)` = `false` に上書きされる |
| 🆕 B-21 [NEW] | `msg.activeTab` の境界チェックなし — タブ数を超えるインデックスをセット可能 |
...
**[NEW] Q-N-18: stateSync で省略フィールドが `false` に上書きされる**
- 場所: `client/src/App.tsx` — `stateSync` メッセージハンドラ
- `EditorMessage` の各フィールドは `?: boolean`（省略可）。`!!(undefined)` = `false` となり、`DEFAULT_TOGGLES` の `verticalHeaderActive: true` が `false` に上書きされる可能性がある。
- **修正案:** `verticalHeaderActive: msg.verticalHeaderActive ?? DEFAULT_TOGGLES.verticalHeaderActive` のようにフォールバックを明示。
...
**[NEW] B-21: `msg.activeTab` の境界チェックなし**
...
3. **[即対応] Q-N-18 (Medium) [NEW]:** `stateSync` ハンドラで `msg.xxx ?? DEFAULT_TOGGLES.xxx` を使用し undefined→false の誤変換を防ぐ。
...
12. **[今週] B-21 (Low) [NEW]:** `setActiveTab(Math.min(msg.activeTab, msg.tabs.length - 1))` に変更。
13. **[今週] B-15 (Low):** SearchPrev/Next ボタンに debounce フラッシュを追加。
14. **[今週] B-18 (Low):** 置換ボタンに debounce フラッシュを追加。
15. **[今週] S-2 (High):** `bridge.ts` の `postMessage` targetOrigin を `window.location.origin` に変更。
16. **[来週・1コマンド] S-18 (Low):** `cd client && npm audit fix` で @babel/core 更新。
17. **[来週] S-24 (Low):** regex モードに入力長上限を追加して ReDoS を緩和。
18. **[来週] S-26 (Low):** renderHtmlValue の auto-link に `rel="noopener noreferrer"` を追加。
19. **[来週] B-7 (Medium):** `doGotoRow` で `Number(rowNum)` を送信。
20. **[来週] Q-N-16 (Low):** 置換ボタンに `title` / `aria-label` を追加。
21. **[来週] Q-N-17 (Low):** `onOpenFile` フォールバックを `onOpenFile?.()` に簡略化。
22. **[来週] S-12 (Medium):** `editor.html` メッセージハンドラに `e.origin` チェックを追加。

> 詳細な過去経緯は `_review/2026-07-25_code_review.md` を参照
