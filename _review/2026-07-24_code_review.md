# コードレビューレポート 2026-07-24 — shunjapanes/main

## サマリー

- 実行日時: 2026-07-24 自動レビュー・3エージェント並列
...
- 発見件数: 🔴 Critical 0 / 🟠 High 6 / 🟡 Medium 17 / 🟢 Low 28 = 計51件
- うち新規 [NEW]: **1件** / 継続 [継続]: **50件** / 解消 [FIXED]: **0件**
- 適用済み自動修正: **0件**（src配下に console.log / console.error / debugger 文なし）

### 今回の主な変化
| 変化 | 内容 |
|------|------|
| 🆕 Q-N-15 [NEW] | SearchBar X（クリア）ボタンに aria-label も title もなし — スクリーンリーダー非対応 |
| ℹ️ コード変更なし | 2026-07-23 以降、src/ への新規コミットなし |
| ℹ️ S-3 攻撃チェーン補足 | S-3 (javascript: XSS) + sessionStorage の PAT → クリック1回でPAT漏洩の連鎖。詳細は下記参照。 |
...
- 場所: `client/src/App.tsx` — `reader.readAsText(file)`
...
- ※ `e.source !== window.parent` チェックは存在するが、悪意ある埋め込み元が `window.parent` になり得るため不十分。
...
- 場所: `client/src/components/SearchBar.tsx:62` — `setTimeout(() => send(...), 30)`
...
- `(a+)+$` 等のパターンでブラウザタブがフリーズ。try/catch はパースエラーのみ捕捉。
...
**[NEW] Q-N-15: SearchBar の X（クリア）ボタンに aria-label も title もなし**
- 場所: `client/src/components/SearchBar.tsx:69` — X アイコンのみの `<button>`
- ChevronUp / ChevronDown / Replace 等の隣接ボタンは `title` を持つが、この X ボタンのみ属性なし。スクリーンリーダーがボタンの目的を読み上げられない。
- **修正案:** `<button ... title="検索クリア" aria-label="検索クリア">`
...
2. **[即対応・1行] S-5 (High):** RibbonToolbar の ToolsTab デバッググループを `{import.meta.env.DEV && ...}` でガード。本番から debugMemo UI に到達不可にする。
3. **[今週] S-7 (High):** iframe に `sandbox="allow-scripts allow-same-origin allow-downloads allow-popups allow-clipboard-read allow-clipboard-write"` 追加。S-11・S-16 も同時解決。
4. **[今週] S-3 (High):** isHtml セルの URL を `https?://` ホワイトリストで検証。PAT漏洩攻撃チェーンを遮断。
5. **[今週・1行] S-8 (Medium):** `App.tsx` メッセージハンドラ先頭に `if (e.origin !== window.location.origin) return;` を追加。
6. **[今週] B-12 (Medium):** SearchBar に `isComposing` チェック追加。日本語 IME 入力中の無効な検索を防止。
7. **[今週] B-8 (Medium):** FileReader に `reader.onerror = () => setStatus('ファイル読み込みエラー')` を追加。
8. **[今週] S-2 (High):** `bridge.ts` の `postMessage` targetOrigin を `window.location.origin` に変更。
9. **[今週・1行] Q-N-15 (Low) [NEW]:** SearchBar X ボタンに `title="検索クリア" aria-label="検索クリア"` を追加。
...
13. **[来週] B-10 (Medium):** Escape/X ハンドラで `clearTimeout(debounceRef.current)` を追加。
14. **[来週] S-12 (Medium):** `editor.html` メッセージハンドラに `e.origin` チェックを追加。
15. **[来週] S-22 (Low):** SW fetch ハンドラでクロスオリジンリクエストをキャッシュスキップ。
16. **[来週] S-14 (Low):** SW catch ブランチで `cached ?? new Response(...)` を返す。
17. **[来週] S-15 (Low):** `handleFileSelected` にファイルサイズ上限チェック (100MB) を追加。
18. **[来週] B-14 (Low):** SearchBar debounce に `if (!query) return;` を追加。
19. **[来週] S-13 (Medium):** sw.js を stale-while-revalidate 戦略に変更。
20. **[来週] S-20 (Low):** sw.js activate ハンドラに旧キャッシュ削除処理を追加。
21. **[来週] Q-N-10 (Low):** `ToggleStates` 型を共有型ファイルに移動し子→親インポートを解消。

> 詳細な過去経緯は `_review/2026-07-23_code_review.md` を参照
